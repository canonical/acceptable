...
class ExtractSchemasFromSourceTests(BuildDoubleTestCase):
    ...
    def test_handles_other_assignments(self):
        self.assertEqual([], _build_doubles.extract_schemas_from_source("foo = {}"))
...
class ExtractSchemasFromFileTests(BuildDoubleTestCase):
    def test_logs_on_missing_file(self):
        ...
        self.assertThat(
            fake_logger.output, Contains("Extracting schemas from %s" % bad_path)
        )
        self.assertThat(
            fake_logger.output,
    ...
    def test_logs_on_no_permissions(self):
        ...
        self.assertThat(
            fake_logger.output, Contains("Cannot extract schemas: Permission denied")
        )
    ...
    def test_logs_on_schema_extraction(self):
        ...
        self.assertThat(
            fake_logger.output, Contains("Extracting schemas from %s" % good_path)
        )
...
class ParseArgsTests(BuildDoubleTestCase):
    def test_error_with_no_args(self):
        self.assertRaises(
            RuntimeError, _build_doubles.parse_args, [], SaneArgumentParser
        )

    def test_scan_file_requires_file(self):
        self.assertRaises(
            RuntimeError, _build_doubles.parse_args, ["scan-file"], SaneArgumentParser
        )
    ...
    def test_build_requires_file(self):
        self.assertRaises(
            RuntimeError, _build_doubles.parse_args, ["build"], SaneArgumentParser
        )
...
class RenderServiceDoubleTests(BuildDoubleTestCase):
    ...
    def test_renders_for_empty_schema_list(self):
        source = _build_doubles.render_service_double("foo", [], "build config-file")
