...
class ServiceMockTests(TestCase):
    ...
    def test_can_construct_double_with_input_schema_and_invalid_payload(self):
        ...
        self.assertEqual(["[] is not of type 'object' at /"], resp.json())
    ...
    def test_mock_records_calls(self):
        ...
        self.assertEqual(json.loads(call1.request.body.decode()), {"call": 1})
        self.assertEqual(json.loads(call2.request.body.decode()), {"call": 2})
    ...
    def test_mock_works_with_multiple_methods(self):
        ...
        self.assertEqual(
            200, requests.post("http://localhost:1234/foo", json={}).status_code
        )
        self.assertEqual(
            200, requests.get("http://localhost:1234/foo", json={}).status_code
        )
        self.assertEqual(
            200, requests.patch("http://localhost:1234/foo", json={}).status_code
        )
    ...
    def test_mock_output_status(self):
        ...
        self.assertEqual(
            201, requests.post("http://localhost:1234/foo", json={}).status_code
        )
