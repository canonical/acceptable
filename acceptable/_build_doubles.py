...
def parse_args(arg_list=None, parser_class=None):
    ...
    build_parser = subparser.add_parser("build", help="build service doubles.")
...
def scan_file(args):
    ...
    print(render_service_double("UNKNOWN", service_schemas, "scan-file %s" % args.file))
...
def build_service_doubles(args):
    with tempfile.TemporaryDirectory() as workdir:
        ...
        for service_name in service_config["services"]:
            ...
            print(
                "Rendered schemas file for %s service: %d schemas"
...
def extract_schemas_from_source(source, filename="<unknown>"):
    ...
    for assign in call_assigns:
        ...
        if (
            isinstance(assign.value.func, ast.Attribute)
            and assign.value.func.value.id in acceptable_services
            and assign.value.func.attr == "api"
        ):
...
def render_value(value):
    ...
    if isinstance(value, list):
        ...
        return (
            "{"
            + ", ".join(
                "{k!r}: {v}".format(k=k, v=render_value(v))
...
def render_service_double(service_name, schemas, regenerate_args):
    ...
    for schema in schemas:
        double_name = "%s_%s" % (schema.view_name, schema.version.replace(".", "_"))
...
def write_service_double_file(target_root, service_name, rendered):
    ...
    target_path = os.path.join(
        target_root, "snapstore_schemas", "service_doubles", "%s.py" % service_name
    )
